...
public class Searching {
    public List<String> findPaths(GLane[] allLanes, Junction[] allJuncs, String src, String dst) {
                ...
                if (element != null) {
                    if (element.strLd.equals(src)) {
                        for (int i = 0; i < 3; i++) {
                            if (element.form[i] > check) {
                                ...
                                if (element.form[i] % 2 == 1) {
                                    ...
                                    hashMap.put(element.strLd + "-E"
                                            + Integer.toString(element.form[i] / 2), 1);
                        ...
                        while (!list.get(0).equals(dst)) {
                            ...
                            if (list2.get(index).charAt(0) == '-'
                                    || list2.get(index).charAt(0) == 'E') {
                                for (int i = 0; i < 3; i++) {
                                    ...
                                    if (allLanes[index1] != null) {
                                        if (allLanes[index1].form[i] > check) {
                                            ...
                                            if (allJuncs[y].allLanes[x] != null
                                                    && allJuncs[y].allLanes[x].strLd != null) {
                                                list.add(allJuncs[y].allLanes[x].strLd);
                                                hashMap.put(list2.get(index)
                                                        + allJuncs[y].allLanes[x].strLd, 1);
                                                hashMap2.put(list.get(list.size() - 1), x);
                                                hashMap3.put(list.get(list.size() - 1), y);
                                            }
                                            ...
                                            if (allLanes[index1].form[i] % 2 == 1) {
                                                list.add("-E" + Integer
                                                        .toString(allLanes[index1].form[i] / 2));
                                                hashMap.put(list2.get(index) + "-E" + Integer
                                                        .toString(allLanes[index1].form[i] / 2), 1);
                                                hashMap3.put(list.get(list.size() - 1),
                                                        allLanes[index1].form[i]);
                                ...
                                for (int i = 0; i < 3; i++) {
                                    ...
                                    if (allJuncs[index1].allLanes[index2].form[i] > check * 2) {
                                        ...
                                        int y = allJuncs[index1].allLanes[index2].form[i]
                                                - (x << 16) - check;
                                        if (allJuncs[y].allLanes[x] != null
                                                && allJuncs[y].allLanes[x].strLd != null) {
                                            list.add(allJuncs[y].allLanes[x].strLd);
                                        ...
                                        if (allJuncs[index1].allLanes[index2].form[i] % 2 == 1) {
                                            list.add("-E" + Integer.toString(
                                                    allJuncs[index1].allLanes[index2].form[i] / 2));
                                            hashMap.put(list2.get(index) + "-E" + Integer.toString(
                                                    allJuncs[index1].allLanes[index2].form[i] / 2),
                                                    1);
                                            hashMap3.put(list.get(list.size() - 1),
                                                    allJuncs[index1].allLanes[index2].form[i]);
                                            if (list.get(0).equals(dst))
                                                break;
                                        } else {
                                            list.add("E" + Integer.toString(
        ...
        int i = list2.size() - 2;
        for (; i > 0; i--) {
            if (hashMap.get(list2.get(i) + list2.get(i + 1)) == null
                    || hashMap.get(list2.get(i) + list2.get(i + 1)) != 1)
                list2.remove(i);
        }
        return list2;
    }
